...
class FeatureExtractor:
    ...
    def _image_transform(self, image_bytes):
        # img = Image.open(path)
        # im = np.array(img).astype(np.float32)
        im = np.array(Image.open(io.BytesIO(image_bytes))).astype(np.float32)
    ...
    def get_detectron_features(self, image_paths):
        ...
        for image_path in image_paths:
            image_url = 'https:' + image_path # image address
            ...
            im, im_scale, im_info = self._image_transform(image_bytes) #image_bytes <- image_path
    ...
    def extract_features(self):
        ...
        if os.path.isfile(self.args.image_dir):

            image_dir_csv = self.args.image_dir
            image_path_df = pd.read_csv(image_dir_csv)
            image_dir = image_path_df['image url'].values
